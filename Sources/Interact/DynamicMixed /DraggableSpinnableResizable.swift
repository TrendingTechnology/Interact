//
//  DraggableSpinnableResizable.swift
//  
//
...
/// # Draggable, Spinnable And Resizable
///  Provides the ability to scale, rotate, drag , and spin the view. Throw the rotation handle and watch it go.
...
public struct DraggableSpinnableResizable: ViewModifier {
    ...
    private func getTopLeading(proxy: GeometryProxy) -> some View{
        self.handle
            ...
            .gesture(
                ...
                .onEnded { (value) in
                    self.viewOffset.width += cos(self.angularVelocity.angle)*value.translation.width/2 - sin(self.angularVelocity.angle)*value.translation.height/2
                    self.viewOffset.height += cos(self.angularVelocity.angle)*value.translation.height/2 + sin(self.angularVelocity.angle)*value.translation.width/2
